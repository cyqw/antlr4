		...
		for (Grammar imp : imports) {
			...
			if ( imp_channelRoot != null) {
				rootGrammar.tool.log("grammar", "imported channels: "+imp_channelRoot);
					...
					for (ANTLRParser.ChannelsSpecContext c: imp_channelRoot) {
						String channel = c.getText();
						boolean channelIsInRootGrammar = false;
						for (ANTLRParser.ChannelsSpecContext rc : channelsRoot) {
							String rootChannel = rc.getText();
...
//
//			// COPY TOKENS
//			List<ANTLRParser.TokensSpecContext> imp_tokensRoot = imp.ast.getTokensSpecs();
//			if ( imp_tokensRoot!=null ) {
//				rootGrammar.tool.log("grammar", "imported tokens: "+imp_tokensRoot);
//				if ( tokensRoot==null ) {
//					tokensRoot = (GrammarAST)adaptor.create(ANTLRParser.TOKENS_SPEC, "TOKENS");
//					tokensRoot.g = rootGrammar;
//					root.insertChild(1, tokensRoot); // ^(GRAMMAR ID TOKENS...)
//				}
//				tokensRoot.add(Arrays.asList(imp_tokensRoot.getChildren().toArray(new Tree[0])));
//			}
//
